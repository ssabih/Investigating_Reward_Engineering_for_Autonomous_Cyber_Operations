...
# Define the FSM for the Reward Machine
full_fsm = {
...
def train_with_seed(seed):
    ...
    register(id='TwoSubnetEnvRM-v0', entry_point='envs:TwoSubnetEnv_RM',)
    env = gym.make('TwoSubnetEnvRM-v0')

    # Initialize agent and RM
    ...
    for e in range(n_episodes):
        ...
        for t in range(15):
            ...
            #print(f"Episode {e+1}, Step {t+1}, Action: {action}, Events: {events}, RM Reward: {rm_reward}, Env Reward: {env_reward}, Total Reward: {total_reward}")
            ...
            avg_env_reward += env_reward

            if done:
                agent.update_target_model()
                agent.completed_episodes += 1
                rewardList.append((e+1, t+1, cumulative_env_reward, done))
                #rewardList.append(cumulative_env_reward)
                #print(f"Episode {e+1}/{n_episodes}, Steps: {t+1}, Score: {cumulative_reward}, Env Score: {cumulative_env_reward}")
                break
    ...
    # Save rewards to CSV
    env_name = 'TwoSubnetEnv_RM'
    filename = 'rm_rewards_' + env_name + '.csv'
    ...
    with open(filename, 'a', newline='') as csvfile:
        ...
        for r in rewardList:
            writer.writerow(['Foundational', 'Reward Machine DQN', seed, r[0], r[1], r[2], r[3]])
            #writer.writerow([r])
