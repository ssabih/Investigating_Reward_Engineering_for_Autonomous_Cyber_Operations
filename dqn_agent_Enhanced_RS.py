    ...
    for e in range(n_episodes):
        ...
        for time in range(50):
            ...
            if len(agent.memory) > batch_size:
                agent.replay()
            if time == 49:
                successtrend.append(0)
                rewardList.append((e+1, time+1, cumulative_reward, done))
                #rewardList.append(cumulative_reward)
                agent.update_target_model()
                #print(f"episode: {e+1}/{n_episodes}, steps: {time+1}, score: {cumulative_reward}")
    # Save results to CSV
    write_header = not os.path.exists('dqn_agent_Enhanced_RS.csv') or os.path.getsize('dqn_agent_Enhanced_RS.csv') == 0
    with open('dqn_agent_Enhanced_RS.csv', 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        if write_header:
            csvwriter.writerow(['env', 'config', 'seed', 'episode', 'steps_in_episode', 'reward_env', 'success'])
        for eachreward in rewardList:
            csvwriter.writerow(['Advanced', 'Shaped DQN', seed, eachreward[0], eachreward[1], eachreward[2], eachreward[3]])
