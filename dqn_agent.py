...
def train_with_seed(seed):
    ...
    write_header = not os.path.exists('dqn_agent.csv') or os.path.getsize('dqn_agent.csv') == 0
    with open('dqn_agent.csv', 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        if write_header:
            csvwriter.writerow(['env', 'config', 'seed', 'episode', 'steps_in_episode', 'reward_env', 'success'])
        for eachreward in rewardList:
            csvwriter.writerow(['Foundational', 'Baseline DQN', seed, eachreward[0], eachreward[1], eachreward[2], eachreward[3]])
